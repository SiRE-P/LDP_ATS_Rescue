...
After 1983, and until 1999, both format of files were available (except for a few corrupted SAS files), and where there were conflicts between the two, the \*.dat values were used.
...
The format is "HH:MM:SS".
All values are local time in Pacific Time (UTC−08 or UTC-07 depending on year and time of year).
In total, 79,012 calculated end times were estimated when no orig_end_time was in the dataset.
...
34 values differ from the original given standardized weight (orig_std_weight_g), indicating potential inconsistencies in the original column.
...
A total of 31,639 calc_std_weight_g values were recalculated and are available where orig_std_weight_g was missing.
A total of 29,828 standardized weights could not be calculated because the preservative information was missing.
...
-   In total, 2,163 invalid formats were detected in the orig_duration_minutes column and flagged in the "data_issues" column.
