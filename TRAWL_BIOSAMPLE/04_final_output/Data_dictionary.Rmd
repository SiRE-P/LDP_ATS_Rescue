...
A total of 29,640 standardized weights could not be calculated because the preservative information was missing.
...
**scale:** : Scale number extracted from original .sas file only, and ranges from 0 to 20.
...
**scale_book:** These numbers range from 1-110, and it is uncertain what this variable represents, as these numbers do not seem to correspond to numbering on physical scale books in our archives.
...
It is flagged in the "data_issues" column as a scale_book_comment: "Weight value was incorrectly recorded in the scale_book field; corrected by re-coding to 0." Any "999" code was replaced by NA.
The letters in this column were from .sas files and these values were transferred to the "scale_book_letter" column.
These are raw values extracted from the original .sas (starting from year 1982) and \*.dat files.
...
Letters from .sas files formerly stored in the "scale_book" column were transferred to this column.
These are raw values extracted from the original .sas (starting from year 1982) and \*.dat files.
...
-   test_trawl_comment: Indicates a suspected test/dummy trawl record.
    ...
    Comment: "Fake orig_start_time, depth_m, orig_duration_minutes and trawl_number. Set to NA."
